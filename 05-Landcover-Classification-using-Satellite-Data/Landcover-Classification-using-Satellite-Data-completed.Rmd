...
```{r}
accuracy = sum(dm$class == dm$veg)/nrow(dm)
error    = sum(dm$class != dm$veg)/nrow(dm)
accuracy
error
```
...
cm = confusionMatrix(data      = dm$class.f, ## predicted classes
                     reference = dm$veg.f,   ## actual    classes
                     positive  = '1')        ## Very necessary!!!!!!
cm
```
...
```{r}
length(which(dm$class == 1 & dm$veg == 1))/
                length(which(dm$veg == 1))
```

Specificity is true negative rate:

```{r}
length(which(dm$class==0 & dm$veg==0))/
              length(which(dm$veg==0))
```
...
for(th in thresholds){
  col.name = paste0("th", th) 
  dth[,col.name] = ifelse(dth$prob>th, 1, 0) ## pred class using th
}
...
Let's summarize the accuracy, sensitivity, specificity, and false positive rate (1-specificity) for each of these thresholds. 
...
```{r}
dpairs = expand.grid(veg1 = d1$prob, 
                     veg0 = d0$prob)
head(dpairs)
```
...
### Deviance Residuals and Deviance

Pearson residual = $\frac{Y_i - \hat{p}_i}{\sqrt{\hat{p}_i(1-\hat{p}_i)}}$

Deviance residual = $\textrm{sign}(Y_i - \hat{p}_i) \sqrt{2\left(Y_i \log\left(\frac{Y_i}{\hat{p}_i}\right) + (1-Y_i)\log\left(\frac{1-Y_i}{1-\hat{p}_i}\right)\right)}$

Deviance residuals for some representative situations.

```{r}
df = data.frame(y = c( 0,  1,  0,  1,  0,  1), 
                p = c(.1, .1, .9, .9, .5, .5)) %>%
  mutate(d.res = sign(y - p)*sqrt(2*(y*log(y/p + 1e-15) + (1-y)*log((1-y)/(1-p) + 1e-15))))
df
```

Here are deviance residuals and deviance for our model. 

```{r}
dm = dm %>% 
  mutate(p.res = (veg - prob)/sqrt(prob*(1-prob)), 
         d.res = sign(veg - prob)*sqrt(2*(
                veg *log(   veg/    prob  + 1e-15) + 
             (1-veg)*log((1-veg)/(1-prob) + 1e-15))))

dev = sum(dm$d.res^2)
dev
m1$deviance ## matches dev

          
```

Null deviance is the deviance for the "null" model, that is, the model with only an intercept. 
...
m0 = glm(veg ~ 1, 
         ...
         family = binomial)
dm = dm %>%
  mutate(prob0 = predict(m0, 
                         newdata = dm, 
                         type = 'response'), 
         d.res0 = sign(veg - prob0)*sqrt(2*(
                veg *log(   veg/    prob0  + 1e-15) + 
             (1-veg)*log((1-veg)/(1-prob0) + 1e-15)))) 
dev0 = sum(dm$d.res0^2)
dev0
m0$deviance ## matches dev0
m1$null.deviance ## matches dev0
...
### Change in probability at the average value for predictor
...
### Slope of the logistic curve at this average value

We could instead find the derivative logistic curve at this average point. If a unit change in $x_1$ is small, then this will be very similar to the previous bullet. Note that the derivative of the logistic curve is 
...
### Maximum change in probability at the inflection point
...
The average change in probability for a unit increase in `NDVI` across all the observations in our data is 0.03. In our situation, this is larger than the change in probability at the average `NDVI100`, because the slope at the average is near 0.

### Summary of all

```{r}
y0 = predict(m1, 
             newdata = data.frame(NDVI100 = 0), 
             type = 'response')
y0
...
g = ggplot(dm, 
  ...
  ## average value of NDVI100
  geom_segment(x    = mean(dm$NDVI100), 
               xend = mean(dm$NDVI100) + 1, 
               y    = p0,
               yend = p1,
               color = pubblue, 
               linewidth = 2) +
  
  ## at inflection point, point of interest
  geom_segment(x    = 0, 
               xend = 1, 
               y    = y0,
               yend = y0 + beta1/4, 
...
g = ggplot(dm, 
  ...
  geom_segment(data = dm %>% slice(8:11), 
               aes(x     = NDVI100 - 0,
                   xend  = NDVI100 + 2,
                   y     = prob - 0*(prob1 - prob),
                   yend  = prob + 2*(prob1 - prob)
                   ),
               color     = pubred,
               linewidth = 1,
               alpha     = 1)
...
```{r fig.height=4, fig.width=4}
ggplot(d, aes(B7, landcover, color=landcover))+
  geom_jitter(height=0.2, width=0)+ 
  theme_pub(type='scatter', base_size=8)
```


```{r}
ggplot(d, aes(B7, NDVI, color=landcover))+
  geom_point()+
  theme_pub(type='scatter', base_size=9)
```

Bands over time for each landcover type. 

```{r}
dd = d %>% 
  filter(!is.infinite(EVI)) %>%
  select(-lat, -lon) %>%
  pivot_longer(cols = c(-ID, -landcover, -year, 
                        -month, -day, -date)) %>%
  mutate(year.mon = year+month/12) %>%
  group_by(landcover, year.mon, name) %>%
  summarise(value=mean(value)) %>%
  rename(Landcover = landcover) %>%
  mutate(Landcover = case_when(Landcover=='builtup'   ~ 'Built-up',
                               Landcover=='cropland'  ~ 'Cropland', 
                               Landcover=='natforest' ~ 'Natural Forest', 
                               Landcover=='orchard'   ~ 'Orchard'))
head(dd)

title = "Intensity of bands over time for each type of landcover" 
g = ggplot(dd, 
           aes(x = year.mon, 
               y = value, 
               color = Landcover))+
